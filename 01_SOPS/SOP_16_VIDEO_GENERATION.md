# SOP 16 — Video Generation

## Purpose
Execute `STAGE_16` repeatably with traceable evidence and a clear gate.

## Required input
Approved package and settings. Optional inputs may include user feedback, style/scene references, and tool constraints. The previous stage must be approved or have a documented blocker/assumption.

## Procedure
1. Read current `STATUS.md`, this SOP, the relevant checklist, and authoritative evidence.
2. Core stage action: register every generation with exact prompt/tool/model/settings/references/output metadata.
...
## Output and gate
Expected output: **video Runs**. Pass requires usable output, sufficient provenance, and the stage-specific quality criteria. Fail requires a blocker/failure record and an explicit repair, regenerate, or return-to-root-stage decision.
