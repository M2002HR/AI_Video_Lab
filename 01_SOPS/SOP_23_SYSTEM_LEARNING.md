# SOP 23 — System Learning

## Purpose
Execute `STAGE_23` repeatably with traceable evidence and a clear gate.

## Required input
Postmortem and evidence. Optional inputs may include user feedback, style/scene references, and tool constraints. The previous stage must be approved or have a documented blocker/assumption.

## Procedure
1. Read current `STATUS.md`, this SOP, the relevant checklist, and authoritative evidence.
2. Core stage action: classify OBS/HYP/EXP/LRN and promote only evidence-supported changes.
...
## Output and gate
Expected output: **learning/change records**. Pass requires usable output, sufficient provenance, and the stage-specific quality criteria. Fail requires a blocker/failure record and an explicit repair, regenerate, or return-to-root-stage decision.
