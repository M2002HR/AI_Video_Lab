# SOP 20 — Post Production

## Purpose
Execute `STAGE_20` repeatably with traceable evidence and a clear gate.

## Required input
Final candidate(s). Optional inputs may include user feedback, style/scene references, and tool constraints. The previous stage must be approved or have a documented blocker/assumption.

## Procedure
1. Read current `STATUS.md`, this SOP, the relevant checklist, and authoritative evidence.
2. Core stage action: record trims, assembly, transitions, color/exposure, text/logo overlays, audio, and delivery format.
...
## Output and gate
Expected output: **edit/composite/assembly record**. Pass requires usable output, sufficient provenance, and the stage-specific quality criteria. Fail requires a blocker/failure record and an explicit repair, regenerate, or return-to-root-stage decision.
