...
## Purpose
Execute `STAGE_17` repeatably with traceable evidence and a clear gate.

## Required input
Video run. Optional inputs may include user feedback, style/scene references, and tool constraints. The previous stage must be approved or have a documented blocker/assumption.

## Procedure
1. Read current `STATUS.md`, this SOP, the relevant checklist, and authoritative evidence.
2. Core stage action: perform frame-by-frame evaluation of identity, morphing, count, contact, gravity, camera, lighting, temporal stability, and final hero.
...
## Output and gate
Expected output: **video QA report**. Pass requires usable output, sufficient provenance, and the stage-specific quality criteria. Fail requires a blocker/failure record and an explicit repair, regenerate, or return-to-root-stage decision.
