# SOP 02 — Product Identity

## Purpose
Execute `STAGE_02` repeatably with traceable evidence and a clear gate.

## Required input
Product images. Optional inputs may include user feedback, style/scene references, and tool constraints. The previous stage must be approved or have a documented blocker/assumption.

## Procedure
1. Read current `STATUS.md`, this SOP, the relevant checklist, and authoritative evidence.
2. Core stage action: document category, silhouette, proportions, materials, texture, color, packaging/components, natural irregularity, uncertainty, and forbidden transformations.
...
## Output and gate
Expected output: **product identity spec, identity lock, uncertainty log**. Pass requires usable output, sufficient provenance, and the stage-specific quality criteria. Fail requires a blocker/failure record and an explicit repair, regenerate, or return-to-root-stage decision.
