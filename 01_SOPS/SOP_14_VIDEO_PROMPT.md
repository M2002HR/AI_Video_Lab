...
اجرای قابل‌تکرار STAGE_14 با حفظ evidence و gate روشن.

## ورودی لازم

identity/reference/scenario/timeline. ورودی اختیاری: feedback کاربر، reference style/scene و constraint ابزار. پیش‌نیاز: stage قبلی approved یا blocker/assumption ثبت‌شده.

## رویه

1. STATUS، SOP/checklist و evidence مرتبط را بخوانید.
2. base logic را با adapter ابزار، نقش رفرنس، physics و final frame ترکیب کن.
...
video prompt package. Pass یعنی usable، provenance کامل و معیار stage؛ Fail یعنی blocker/failure و تصمیم repair/regenerate/بازگشت ثبت شده.
