...
## Purpose
Execute `STAGE_10` repeatably with traceable evidence and a clear gate.

## Required input
Shot design. Optional inputs may include user feedback, style/scene references, and tool constraints. The previous stage must be approved or have a documented blocker/assumption.

## Procedure
1. Read current `STATUS.md`, this SOP, the relevant checklist, and authoritative evidence.
2. Core stage action: define composition, subject, action, and transition for every controlling beat.
...
## Output and gate
Expected output: **storyboard specification**. Pass requires usable output, sufficient provenance, and the stage-specific quality criteria. Fail requires a blocker/failure record and an explicit repair, regenerate, or return-to-root-stage decision.
